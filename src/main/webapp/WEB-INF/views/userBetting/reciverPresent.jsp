 	...
 	<c:when test="${'reciver' eq presentType }">
    	...
    	<c:if test="${not empty presents }">
   		<c:forEach items="${presents }" var="present">
             ...
             <form action="${pageContext.request.contextPath }/select/selectReciverPresentDetails.do" method="post">
               	<input type="hidden" name="userName" value="${present.userName }"/>
   		...
   		<p class="user-pages">
   				<c:if test="${nowPage >1 }">
					<a href="${pageContext.request.contextPath }/select/selectReciverPresent.do?nowPage=${nowPage-1}&startLine=${(nowPage-2)*maxLine}&endLine=${(nowPage-1)*maxLine}">上一页</a>
    			</c:if>
    			<c:choose>
    				<c:when test="${totalPage>1&&(totalPage<5 || totalPage==5) }">
    					<c:forEach var="item" varStatus="status" begin="1" end="${totalPage }">
    						<c:choose>
    							...
    							<c:otherwise>
    								<a href="${pageContext.request.contextPath }/select/selectReciverPresent.do?nowPage=${status.index}&startLine=${(status.index-1)*maxLine}&endLine=${status.index*maxLine}">${ status.index}</a>
    							</c:otherwise>
    						...
    						<c:when test="${nowPage<totalPage-3 }">
    							<c:choose>
    								<c:when test="${nowPage>3 }">
    									<a href="${pageContext.request.contextPath }/select/selectReciverPresent.do?nowPage=1&startLine=0&endLine=${maxLine}">1</a><a 
    						   		   	  >…</a><a 
    						   		       href="${pageContext.request.contextPath }/select/selectReciverPresent.do?nowPage=${nowPage-1}&startLine=${(nowPage-3)*maxLine}&endLine=${(nowPage-2)*maxLine}">${nowPage-1 }</a><a class="cur-page">${nowPage }</a><a 
    						   		       href="${pageContext.request.contextPath }/select/selectReciverPresent.do?nowPage=${nowPage+1}&startLine=${(nowPage)*maxLine}&endLine=${(nowPage+1)*maxLine}">${nowPage+1 }</a><a 
    						   		      >…</a><a 
    						   		       href="${pageContext.request.contextPath }/select/selectReciverPresent.do?nowPage=${totalPage}&startLine=${(totalPage-1)*maxLine}&endLine=${totalPage*maxLine}">${totalPage }</a>
    								</c:when>
    								<c:otherwise>
    			...
    			<c:if test="${totalPage >nowPage }">
    				<a href="${pageContext.request.contextPath }/select/selectReciverPresent.do?nowPage=${nowPage+1}&startLine=${nowPage*maxLine}&endLine=${(nowPage+1)*maxLine}">下一页</a>
    			</c:if>
    		</p>
   	</c:if>
