...
@Component
public class BetCodeAnalysisUtil {
	/**
	 * 解析betCode
	 * @param lotNo
	 * @param betCode
	 * @return
	 */
	public String betCodeAnalysis(String lotNo,String betCode){
		String betCodeView = "";
		String betType = "";
		if(lotNo!=null&&!"".equals(lotNo)&&Constants.LOTNO_QLC.equals(lotNo)){//七乐彩
			...
			String[] betCodes = betCode.split("!");
			for(String bet : betCodes) {
				if (bet.substring(0, 2).equals("20")) { //直选复式 20020102^0106^0100^
					String[] codes = bet.split("\\^");
					...
					for (String code : codes) {
						...
						if (play.trim().equals("DXD") || play.trim().equals("Z3D") || play.trim().equals("Z6D")) { //单式
							betCodeView +="玩法:"+betType+"<br/>"+ LotteryUtil.getDouHaoStringFromStringList(LotteryUtil.getStringListFromZeroString(code.substring(4, code.length())))+"<br/>";
						} else if (play.trim().equals("Z3F") || play.trim().equals("Z6F") || play.trim().equals("DXDF")) { //复式
							betCodeView +="玩法:"+betType+"<br/>"+LotteryUtil.getDouHaoStringFromStringList(LotteryUtil.getStringListFromZeroString(code.substring(6)))+"<br/>";
...
//							jsonObject.put("betType", "胆拖");
			...
			for(String bet : betCodes) {
				...
				for (String code : codes) {
					...
					if (split3[0].equals("5D")) { //五星
						...
						betCodeView+= "玩法:"+betType+"<br/>"+"十位:"+shiStr+"|个位:"+geStr+"<br/>";
						continue;
			...
			String[] betCodes = betCode.split("!");
			for(String bet : betCodes) {
				String[] codes = bet.split("@"); //102@*02040511^
			...
			for (String bet : betCodes) {
				String codes[] = bet.split("\\|");
				betType = getGuangDongHappyTenMinutesType(codes[0], codes[1]);
				if("S".equals(codes[0])||"M".equals(codes[0])){
				...
				}else if("P".equals(codes[0])){
					String pStr[] = codes[0].split("\\+");
					if("Q2".equals(codes[1])){
		...
		}else if(lotNo!=null&&!"".equals(lotNo)&&Constants.LOTNO_QLC.equals(lotNo)){
...
//		}
	...
	public static String convertBetCodeFor11YDJ_DWFS(String betCode) { 
		...
		if (split.length==2) { //142@04*050607^
			...
			sBuffer.append("万位:"+LotteryUtil.getDouHaoStringFromStringList(LotteryUtil.getStringListFromZeroString(wan))
					+"<br/>千位:"+LotteryUtil.getDouHaoStringFromStringList(LotteryUtil.getStringListFromZeroString(qian)));
			...
			sBuffer.append("万位:"+LotteryUtil.getDouHaoStringFromStringList(LotteryUtil.getStringListFromZeroString(wan))
					+"<br/>千位:"+LotteryUtil.getDouHaoStringFromStringList(LotteryUtil.getStringListFromZeroString(qian))
					+"<br/>百位:"+LotteryUtil.getDouHaoStringFromStringList(LotteryUtil.getStringListFromZeroString(bai)));
		}
//		String string = sBuffer.toString();
//		if (string.endsWith("\n")) {
//			return string.substring(0, string.length()-1);
